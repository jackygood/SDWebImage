...
+(UIImage *)compressImageWith:(UIImage *)image
{
    if (image.size.width > 300) {
        ...
        image = [ImageUtil image:image fitInSize:CGSizeMake(300, 300)];
    }
...
#ifdef DEBUG
            NSData * imageData = UIImageJPEGRepresentation(frameImage, 1);
            NSLog(@"BEFORE:gif:%d;;data size::%.2fKB\nwidth::%.2f",i,imageData.length/1024.0,frameImage.size.width);
#else
            
#endif
            frameImage = [FLAnimatedImage compressImageWith:frameImage];
            
            
#ifdef DEBUG
            imageData = UIImageJPEGRepresentation(frameImage, 1);
            NSLog(@"END:gif:%d;;data size::%.2fKB\nwidth::%.2f",i,imageData.length/1024.0,frameImage.size.width);
